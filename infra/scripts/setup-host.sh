...
echo "== Repositório =="
if [ ! -d /opt/dindin ]; then
  sudo mkdir -p /opt/dindin
  sudo chown "$USER":"$USER" /opt/dindin
  git clone https://github.com/JokerFoxyy/DinDin.git /opt/dindin
fi

echo
echo "Próximos passos manuais:"
echo "1. cp /opt/dindin/infra/.env.prod.example /opt/dindin/infra/.env e preencher com valores reais"
echo "2. cd /opt/dindin && docker compose -f infra/docker-compose.prod.yml --env-file infra/.env up -d"
echo "3. ./infra/scripts/configure-s3-lifecycle.sh <bucket>  (uma vez, após criar o bucket)"
echo "4. Agendar infra/scripts/backup.sh no cron (ex.: crontab -e -> 0 3 * * * /opt/dindin/infra/scripts/backup.sh)"
